...
TMP_DB::TMP_DB() {
    ...
    rc |= sqlite3_exec(queue_db,
                       "CREATE TABLE queue(id INTEGER PRIMARY KEY, status TEXT, timestamp INTEGER DEFAULT (strftime('%s','now')));",
                       nullptr,
                       nullptr,
                       nullptr);
    ...
    rc |= sqlite3_exec(resource_db, "CREATE TABLE slot(id INTEGER PRIMARY KEY, status TEXT);", nullptr, nullptr,
                       nullptr);
